...
Directory Service хранит базовые справочники оргструктуры — подразделения, должности и локации — и предоставляет единый CRUD‑интерфейс для всех внутренних сервисов. Благодаря этому данные не дублируются, а структура компании остаётся консистентной.

В экосистеме работают десятки сервисов. Всем им периодически требуются одни и те же сущности. Если каждый сервис будет хранить эти справочники отдельно, названия неизбежно разойдутся, а закрытые отделы «оживут» в чужих БД.
...
Это “мини‑система” для описания того, **какие отделы есть в компании, где они расположены и какие должности в них бывают**. Всё это — с поддержкой сложной структуры, историей и гибкостью для будущих изменений.
